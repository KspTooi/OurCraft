 ...
 */
@Slf4j
public class ClientNetworkRouter {

    //定义 RPC 处理器接口
    @FunctionalInterface
    public interface RpcPacketHandler<T> {
        void handle(ClientNetworkSession session, T packet, long rpcId);
    }

    // 存储 数据包类型 -> 处理器列表 的映射
    // BiConsumer<Session, Packet> 让处理器能同时访问会话和数据包
    private final Map<Class<?>, List<BiConsumer<ClientNetworkSession, ?>>> listeners = new HashMap<>();
     ...
     * @param <T> 数据包类型
     */
    public <T> void subscribe(Class<T> packetClass, BiConsumer<ClientNetworkSession, T> listener) {
    ...
    public void post(ClientNetworkSession session, Object packet) {
        List<BiConsumer<ClientNetworkSession, ?>> handlers = listeners.get(packet.getClass());

        if (handlers != null) {
            for (BiConsumer<ClientNetworkSession, ?> handler : handlers) {
                try {
                    // 强制类型转换是安全的，因为 subscribe 时保证了类型匹配
                    ((BiConsumer<ClientNetworkSession, Object>) handler).accept(session, packet);
                } catch (Exception e) {
     ...
     */
    @SuppressWarnings("unchecked")
    public void postRpc(ClientNetworkSession session, Object packet, long rpcId) {
        // 查找 RPC 处理器
