...
public class FlexChunkMeshGenerator {
    ...
    public void submitMeshTask(FlexClientChunk chunk) {
    ...
    public MeshGenerationResult calculateMeshData(FlexClientChunk chunk, ClientWorld clientWorld) {
        ...
        for (int x = 0; x < FlexClientChunk.CHUNK_SIZE; x++) {
            for (int y = 0; y < chunkHeight; y++) {
                for (int z = 0; z < FlexClientChunk.CHUNK_SIZE; z++) {
                    // 使用快照无锁读取方块状态
                    ...
                    int worldX = chunkX * FlexClientChunk.CHUNK_SIZE + x;
                    int worldZ = chunkZ * FlexClientChunk.CHUNK_SIZE + z;
    ...
    private boolean shouldRenderFace(ClientWorld clientWorld, int x, int y, int z, int dx, int dy, int dz, 
        ...
        // 计算邻居方块所在的区块坐标
        int neighborChunkX = (int) Math.floor((float) neighborX / FlexClientChunk.CHUNK_SIZE);
        int neighborChunkZ = (int) Math.floor((float) neighborZ / FlexClientChunk.CHUNK_SIZE);
        
        BlockState neighborState;
        
        // 如果邻居在本区块内，使用快照无锁读取
        if (neighborChunkX == currentChunk.getChunkX() && neighborChunkZ == currentChunk.getChunkZ()) {
            int localX = neighborX - neighborChunkX * FlexClientChunk.CHUNK_SIZE;
            int localZ = neighborZ - neighborChunkZ * FlexClientChunk.CHUNK_SIZE;
