...
public class JmeClientPlayer extends JmeClientLivingEntity {
    ...
    public JmeClientPlayer(JmeClientWorld world) {
        super(UUID.randomUUID(), world);
    ...
    public JmeClientPlayer(UUID uniqueId, JmeClientWorld world) {
    ...
    public void handleMouseInput(float deltaX, float deltaY) {
        if (deltaX != 0 || deltaY != 0) {
            // deltaX 和 deltaY 已经在 GameplayState 中应用了灵敏度
            yaw += deltaX;
            pitch += deltaY;
