...
在 `GameServer.java` 中，我们将移除 `processInputEvents()` 和 `processSystemEvents()` 这两个独立的方法，并用一个统一的 `processEvents()` 方法取而代之。
    ...
    private void processEvents() {
        java.util.List<com.ksptool.mycraft.sharedcore.events.GameEvent> events = 
            com.ksptool.mycraft.sharedcore.events.EventQueue.getInstance().pollAll();
        
        for (com.ksptool.mycraft.sharedcore.events.GameEvent event : events) {
            // 系统事件
            if (event instanceof com.ksptool.mycraft.sharedcore.events.ClientReadyEvent) {
                performInitialSync();
            }
            // 玩家输入事件
            else if (event instanceof com.ksptool.mycraft.sharedcore.events.PlayerInputEvent) {
                handlePlayerInput((com.ksptool.mycraft.sharedcore.events.PlayerInputEvent) event);
            } else if (event instanceof com.ksptool.mycraft.sharedcore.events.PlayerHotbarSwitchEvent) {
                handlePlayerHotbarSwitch((com.ksptool.mycraft.sharedcore.events.PlayerHotbarSwitchEvent) event);
            } else if (event instanceof com.ksptool.mycraft.sharedcore.events.PlayerMouseClickEvent) {
                handlePlayerMouseClick((com.ksptool.mycraft.sharedcore.events.PlayerMouseClickEvent) event);
            }
        }
    }
...
`Player.applyInput()` 方法需要访问世界的 `ticksPerSecond` 属性来正确计算加速度，而该属性位于 `WorldTemplate` 中。
    ...
    public interface IWorld {
        // ... existing methods
        com.ksptool.mycraft.world.WorldTemplate getTemplate();
    }
    ```
