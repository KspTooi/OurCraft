...
在之前的客户端-服务端渲染分离重构之后，代码库中依然存在一个残留的、未被使用的 `ChunkMeshGenerator.java` 文件。该文件位于 `com.ksptool.mycraft.world` 包下，是旧架构的遗物。当前的实现已经将该类的职责完全移交给了位于 `com.ksptool.mycraft.client.world` 包下的同名类。为了维护代码的整洁和避免未来的混淆，我们将删除这个多余的文件。
...
2.  **`Chunk.java` 的问题**:
    ...
    *   **第 5 行**: `import com.ksptool.mycraft.client.rendering.TextureManager;`
...
当前架构最大的问题在于 `com.ksptool.mycraft.world.Chunk` 类混合了服务端的数据逻辑和客户端的渲染逻辑，特别是 `calculateMeshData` 方法及其对客户端 `TextureManager` 的依赖。本计划将通过以下步骤彻底解决此问题：
...
-   **操作**:
    ...
    3.  修改文件内的包声明为 `package com.ksptool.mycraft.server.world;`。
    ...
    5.  **关键清理**: 从 `ServerChunk.java` 中彻底删除以下内容：
        ...
        -   `BlockFace` 枚举。
        -   对 `com.ksptool.mycraft.client.rendering.TextureManager` 的导入语句。
...
-   **操作**: 这是最关键的一步，需要细致地修改所有引用旧 `Chunk` 类的地方。
    -   **服务端**: 在所有服务端相关的类中（例如 `ServerWorld`, `GameServer`, `world.ChunkManager`, `world.save.*`, `world.gen.*`），将 `import com.ksptool.mycraft.world.Chunk;` 替换为 `import com.ksptool.mycraft.server.world.ServerChunk;`，并将所有 `Chunk` 类型变量改为 `ServerChunk`。
