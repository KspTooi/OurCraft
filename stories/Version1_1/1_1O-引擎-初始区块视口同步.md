...
我们将创建一个新方法 `performInitialSync()`，该方法负责遍历玩家初始位置周围的区块，并为它们提交网格生成任务。
    ...
    private void performInitialSync() {
        ...
        log.info("执行初始视口同步...");
        int playerChunkX = (int) Math.floor(player.getPosition().x / com.ksptool.mycraft.world.Chunk.CHUNK_SIZE);
        int playerChunkZ = (int) Math.floor(player.getPosition().z / com.ksptool.mycraft.world.Chunk.CHUNK_SIZE);
        ...
        for (int x = playerChunkX - INITIAL_RENDER_DISTANCE; x <= playerChunkX + INITIAL_RENDER_DISTANCE; x++) {
            for (int z = playerChunkZ - INITIAL_RENDER_DISTANCE; z <= playerChunkZ + INITIAL_RENDER_DISTANCE; z++) {
                com.ksptool.mycraft.world.Chunk chunk = world.getChunk(x, z);
                if (chunk != null) {
...
最后，客户端需要响应该卸载事件，清理不再需要的区块数据和渲染资源。
    ...
    public void processEvents() {
        for (GameEvent event : events) {
            // ...
            if (event instanceof com.ksptool.mycraft.sharedcore.events.ChunkUnloadEvent) {
                handleChunkUnload((com.ksptool.mycraft.sharedcore.events.ChunkUnloadEvent) event);
            }
        }
    }
    
    private void handleChunkUnload(com.ksptool.mycraft.sharedcore.events.ChunkUnloadEvent event) {
